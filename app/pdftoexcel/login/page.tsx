...
export default function PdfToExcelLogin() {
  return (
...
function LoginForm() {
  ...
  const from = params.get('from') || '/pdftoexcel';
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const res = await fetch('/api/pdftoexcel-login', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        // creada y que el middleware se evalúe de cero.
        const target = from.startsWith('/pdftoexcel') ? from : '/pdftoexcel';
  ...
  return (
    <main className="auth-wrap">
      <form className="auth-card" onSubmit={handleSubmit}>
        ...
        <h1 className="auth-title">PDF to Excel</h1>
